...
Me permito hacer una cita traduciendo el concepto (lo más básico posible) de la [nota introductoria del propio Hafen](https://ryanhafen.com/blog/geofacet/): 

> "El paquete `geofacet` amplía `ggplot2` de una manera que facilita la creación de visualizaciones geográficamente facetadas en `R`. Para hacer `geofacet` se toman datos que representan diferentes entidades geográficas y se aplica un método de visualización para cada entidad, con el conjunto resultante de visualizaciones establecidas en una cuadrícula que imita la topología geográfica original lo más cerca posible".

A continuación presetnare con un ejemplo sencillo cuales (creo) son las mayores ventajas de `geofacet`. A saber:

* se pueden graficar múltiples variables para cad unidad geográfica
...
---
### Nùmero Efectivo de Partidos (NEP)

Vamos a usar datos de *NEP* en base a datos del atlas de Calvo & Escolar y [\@andy_tow](http://twitter.com/andy_tow). El indicador computa la proporción de votos que el partido $_i$ obtiene sobre el total de votos emitidos:

$\LARGE N_{2}=\frac{1}{\sum_{i}^{n}p_{i}^2}$

Los oirginales calculados por [\@Crst_C](http://twitter.com/Crst_C/) [^1] están disponibles en [este post](http://observablesyhechos.blogspot.com/2014/08/numero-efectivo-de-partidos-en.html).

[^1]: [_Laakso & Taagepera (1979)_](https://en.wikipedia.org/wiki/Effective_number_of_parties)  
...
```{r geogridARG, echo=TRUE, message=FALSE, warning=FALSE}
